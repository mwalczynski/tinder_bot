...
class JsonWrapper:
    ...
    def _wrap(self, key, value, no_key=False):
        if value is not MISSING_VALUE and isinstance(value,
                                                     (dict, list, tuple)):
    ...
    def _get_value(self, item):
        ...
        elif (isinstance(self._data, (list, tuple)) and isinstance(item, int)
              and -len(self._data) <= item < len(self._data)):
    ...
    def __iter__(self):
        if isinstance(self._data, dict):
            yield from (self._data
                        if self.iter_keys_only else self._data.items())
