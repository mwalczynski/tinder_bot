...
class MatchUser(UserModel):
    def __init__(self, uid, match_id, name, bio, age, birth_date, photos,
                 gender, distance, job_name, job_title, school_name, school_id,
                 ping_time, top_song, instagram_photos):
        super().__init__(uid, name, bio, age, birth_date, photos, gender,
                         distance, job_name, job_title, school_name, school_id,
                         ping_time, top_song, instagram_photos)
    ...
    def get_messages(self):
        ...
        return [
            Message(x, self.id, self.name) for x in self.match_data['messages']
        ]
...
class Message():
    ...
    def is_liked(self):
        ...
        liked_messages = [
            x['message_id']
            for x in session.Session().get_updates()['liked_messages']
        ]
...
class UserController:
    ...
    def get_user(self):
        ...
        instagram_photos = [
            photo.image for photo in self.const.instagram.photos
        ]
        if self.user_type is 'Normal':
            return NormalUser(self.id, name, bio, age, birth_date, photos,
                              gender, distance, job_name, job_title,
                              school_name, school_id, ping_time, top_song,
                              instagram_photos)
        elif self.user_type is 'Match':
            return MatchUser(self.id, self.match_id, name, bio, age,
                             birth_date, photos, gender, distance, job_name,
                             job_title, school_name, school_id, ping_time,
                             top_song, instagram_photos)
        elif self.user_type is 'Me':
            return UserModel(self.id, name, bio, age, birth_date, photos,
                             gender, distance, job_name, job_title,
                             school_name, school_id, ping_time, top_song,
                             instagram_photos)
    ...
    def _decode_age(self):
        ...
        return (today.year - self._decode_birth_date().year - (
            ...
            (self._decode_birth_date().month, self._decode_birth_date().day)))
